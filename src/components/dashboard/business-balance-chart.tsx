...
import type { BusinessBalanceYear } from "@/lib/analytics/business-balance";
...
export function BusinessBalanceChart({ data }: { data: BusinessBalanceYear[] }) {
  if (data.length === 0) return null;

  const chartData = data.map((y) => {
    const row: Record<string, string | number> = { fy: y.fy, income: y.income };
  ...
  const hasLoss = data.some((y) => y.segments.some((s) => s.key === "profit" && s.percent < 0));
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Every revenue dollar by financial year, from the Xero P&amp;L. Direct labour is the
          wage and superannuation cost inside cost of sales; overhead salaries sit in Overheads.
        </p>
      </CardHeader>
      <CardContent className="space-y-3">
        <ResponsiveContainer width="100%" height={340}>
          <BarChart data={chartData} margin={{ top: 12, right: 16, bottom: 4, left: 4 }} barCategoryGap="28%">
            <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-muted" />
            <XAxis dataKey="fy" tick={{ fontSize: 12 }} tickLine={false} />
            <YAxis
              tick={{ fontSize: 11 }}
              tickLine={false}
              axisLine={false}
        ...
        {hasLoss && (
          <p className="text-xs text-muted-foreground">
            ...
            then exceed 100% of revenue, which is what a loss-making year looks like.
          </p>
        )}
      </CardContent>
    </Card>
